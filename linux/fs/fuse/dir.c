...
static void fuse_lookup_end(struct fuse_mount *fm, struct fuse_args *args, int error)
{
	pr_debug("%s: fm=0x%p, nodeid=%lld, name=%s, len=%d\n", __func__, fm, args->nodeid, (char *)args->in_args[0].value, args->in_args[0].size-1);

	kfree(args->in_args[0].value);
	kfree(args->out_args[0].value);
	kfree(args);
}

static void fuse_lookup_background(struct fuse_mount *fm, u64 nodeid, const struct qstr *name)
{
	struct fuse_args *args;
	struct fuse_entry_out *outarg;
	unsigned char *value;

	pr_debug("%s: fm=0x%p, nodeid=%lld, name=%s, len=%d\n", __func__, fm, nodeid, name->name, name->len);

	args = kzalloc(sizeof(struct fuse_args), GFP_KERNEL);
	if (!args) {
		return;
	}
	outarg = kzalloc(sizeof(struct fuse_entry_out), GFP_KERNEL);
	if (!args) {
		kfree(args);
		return;
	}
	value = kzalloc((name->len + 1) * sizeof(unsigned char), GFP_KERNEL);
	if (!value) {
		kfree(args);
		kfree(outarg);
		return;
	}
	memcpy(value, name->name, name->len);

	args->opcode = FUSE_LOOKUP;
	args->nodeid = nodeid;
	args->in_numargs = 1;
	args->in_args[0].size = name->len + 1;
	args->in_args[0].value = value;
	args->out_numargs = 1;
	args->out_args[0].size = sizeof(struct fuse_entry_out);
	args->out_args[0].value = outarg;
	args->end = fuse_lookup_end;
	fuse_simple_background(fm, args, GFP_KERNEL);
}
...
static int fuse_dentry_revalidate(struct dentry *entry, unsigned int flags)
{
	...
	if (inode && fuse_is_bad(inode))
		...
		nodeid = get_node_id(d_inode(parent));

		fuse_lookup_background(fm, nodeid, &entry->d_name);
		ret = fuse_phash_get_entry_out(fm->fc, nodeid, entry->d_name.name, entry->d_name.len, &outarg);
		if (ret != -ENOENT && (ret < 0 || !outarg.nodeid)) {
			fuse_lookup_init(fm->fc, &args, nodeid,
					&entry->d_name, &outarg);
		     ...
		     struct fuse_entry_out *outarg, struct inode **inode)
{
	...
	*inode = NULL;
	...
	fuse_lookup_background(fm, nodeid, name);
	err = fuse_phash_get_entry_out(fm->fc, nodeid, name->name, name->len, outarg);
	if (err == -ENOENT)
		goto out_put_forget;
