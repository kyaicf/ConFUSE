...
static inline bool fuse_is_io_cache_wait(struct fuse_inode *fi)
{
	...
	return READ_ONCE(fi->iocachectr) < 0 && (!fb || fuse_backing_partial(fb));
}
...
static int fuse_inode_backing_update(struct fuse_inode *fi, struct fuse_backing *fb)
{
	pr_debug("%s: fi=0x%p, fb=0x%p\n", __func__, fi, fb);
...
static int fuse_file_passthrough_open(struct inode *inode, struct file *file)
{
	...
	if (!IS_ENABLED(CONFIG_FUSE_PASSTHROUGH) || !fc->passthrough ||
	    (ff->open_flags & ~FOPEN_PASSTHROUGH_MASK & ~FOPEN_KEEP_CACHE))
		return -EINVAL;
	...
	if (fuse_backing_partial(fb)) {
		pr_debug("%s: inode=0x%p partial, enter cached\n", __func__, inode);
		...
		if (err)
			goto err_out;
		err = fuse_inode_backing_update(fi, fb);
		if (err) {
			fuse_file_cached_io_release(ff, fi);
			goto err_out;
		}
	} else {
		pr_debug("%s: inode=0x%p not partial, enter uncached\n", __func__, inode);

		/* uncached io not allowed FOPEN_KEEP_CACHE */
