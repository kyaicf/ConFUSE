...
struct fuse_conn {
	...
	/** Perfect hash for lookup operation */
	struct fuse_phash *fph;
	wait_queue_head_t fph_waitq;
};
...
static inline bool fuse_backing_partial(struct fuse_backing *fb)
{
	return fb ? (fb->flags & FUSE_BACKING_PARTIAL) : false;
}
...
/**
 * struct fuse_backing_span_iter - Find used and unused spans.
...
#define fuse_backing_for_each_span(span, ff, first_index, last_index)      \
	for (fuse_backing_span_iter_first(span, ff,                        \
					  first_index, last_index);        \
	     !fuse_backing_span_iter_done(span);                           \
	     fuse_backing_span_iter_next(span))

int fuse_passthrough_pages_io_init(void);
...
static inline struct fuse_phash *fuse_conn_phash(struct fuse_conn *fc)
{
	return READ_ONCE(fc->fph);
}
