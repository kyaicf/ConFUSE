...
// TODO: delete
static long fuse_dev_ioctl_backing_intervals_set(struct file *file, struct fuse_backing_intervals_set __user *argp)
{
	pr_debug("%s: file=0x%p\n", __func__, file);
...
static long fuse_dev_ioctl_metadata_set(struct file *file, __u32 __user *argp)
{
	pr_debug("%s: file=0x%p\n", __func__, file);

	struct fuse_dev *fud = fuse_get_dev(file);
	unsigned int metadata_fd;

	if (!fud)
		return -EPERM;

	if (get_user(metadata_fd, argp))
		return -EFAULT;

	return fuse_phash_init(fud->fc, metadata_fd);
}
			   ...
			   unsigned long arg)
{
	...
	// TODO: delete
	case FUSE_DEV_IOC_BACKING_INTERVALS_SET:
		return fuse_dev_ioctl_backing_intervals_set(file, argp);

	case FUSE_DEV_IOC_METADATA_SET:
		return fuse_dev_ioctl_metadata_set(file, argp);
