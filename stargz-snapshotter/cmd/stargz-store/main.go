...
const (
	...
	defaultConfigPath = "/etc/passthrough-store/config.toml"
	defaultRootDir    = "/var/lib/passthrough-store"
)
...
func main() {
	...
	if config.Config.FuseConfig.Passthrough {
		if config.Config.FuseConfig.BackingfileStorageDir == "" {
			config.Config.FuseConfig.BackingfileStorageDir = path.Join(*rootDir, "bfStorage")
		}
	}
	config.Config.FuseConfig.StargzStoreMode = true
	...
	if err := store.Mount(ctx, mountPoint, layerManager, config.Config); err != nil {
...
func getMetadataStore(rootDir string, config Config) (metadata.Store, error) {
	...
	case dbMetadataType:
		...
		return func(sr *io.SectionReader, async bool, opts ...metadata.Option) (metadata.Reader, error) {
