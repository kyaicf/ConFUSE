...
import (
	...
	lz4chunkedconvert "github.com/containerd/stargz-snapshotter/nativeconverter/lz4chunked"
...
func measureTime(start time.Time) {
	...
	fmt.Printf("Convert image  took %v to complete.\n", duration)
}
...
`,
	Flags: []cli.Flag{
		...
		cli.BoolFlag{
			Name:  "lz4chunked",
			Usage: "use lz4 compression instead of gzip (a.k.a lz4:chunked). Must be used in conjunction with '--oci'.",
		},
		cli.IntFlag{
			Name:  "lz4chunked-chunk-size",
			Usage: "lz4:chunked chunk size",
			Value: 0,
		},
		// generic flags
		cli.BoolFlag{
	...
	Action: func(context *cli.Context) error {
		...
		if context.Bool("lz4chunked") {
			esgzOpts, err := getLz4chunkedConvertOpts(context)
			if err != nil {
				return err
			}
			layerConvertFunc = lz4chunkedconvert.LayerConvertFuncWithCompressionLevel(
				0, esgzOpts...)
			if !context.Bool("oci") {
				return errors.New("option --lz4chunked must be used in conjunction with --oci")
			}
			if context.Bool("uncompress") {
				return errors.New("option --lz4chunked conflicts with --uncompress")
			}
		}
...
func getLz4chunkedConvertOpts(context *cli.Context) ([]estargz.Option, error) {
	esgzOpts := []estargz.Option{
		estargz.WithChunkSize(context.Int("lz4chunked-chunk-size")),
	}
