...
var OptimizeCommand = cli.Command{
	...
	Flags: append([]cli.Flag{
		...
		cli.BoolFlag{
			Name:  "lz4chunked",
			Usage: "use lz4 compression instead of gzip (a.k.a lz4:chunked)",
		},
		cli.UintFlag{
			Name:  "lz4chunked-compression-level",
			Usage: "lz4:chunked compression level",
			Value: 0, // SpeedDefault; see also https://pkg.go.dev/github.com/klauspost/compress/zstd#EncoderLevel
		},
	}, samplerFlags...),
	Action: func(clicontext *cli.Context) error {
		...
		} else if clicontext.Bool("zstdchunked") || clicontext.Bool("lz4chunked") {
			return errors.New("option --zstdchunked or --lz4chunked must be used in conjunction with --oci")
		}
		...
		} else if clicontext.Bool("lz4chunked") {
			f = lz4chunkedconvert.LayerConvertWithLayerOptsFuncWithCompressionLevel(
				lz4.CompressionLevel(clicontext.Uint("lz4chunked-compression-level")), esgzOptsPerLayer)
