...
func Mount(ctx context.Context, mountpoint string, layerManager *LayerManager, cfg config.Config) error {
	timeSec := time.Second
	...
	if err != nil {
		log.G(ctx).WithError(err).Debugf("fs mount, GetBackFileDb ", fusermountBin)
		return err
	}
