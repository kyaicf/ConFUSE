...
func (b *rawBridge) newInodeUnlocked(ops InodeEmbedder, id StableAttr, persistent bool) *Inode {
	...
	//保证toc生成的id和inode id 一致
	initInode(ops.embed(), ops, id, b, persistent, id.Ino)
	b.nextNodeId++
	return ops.embed()
}
...
func (b *rawBridge) Nopen(nid uint64) int {
	b.mu.Lock()
	defer b.mu.Unlock()
	n := b.kernelNodeIds[nid]
	return len(n.openFiles)
}
...
func (b *rawBridge) inode(id uint64, fh uint64) (*Inode, *fileEntry) {
	...
	//getParent need lock for adding inode id
	b.getParent(id)
...
// TODO group layer id
func (b *rawBridge) getParent(id uint64) {
	...
	b.mu.Lock()
	root := b.kernelNodeIds[fuse.FUSE_ROOT_ID]
	b.mu.Unlock()

	if lu, ok := root.ops.(NodeInodeGeter); ok {
		out := &fuse.EntryOut{}
		parent, name, child, errno := lu.GetInodes(id, out)
		if errno != 0 {
			if b.options.NegativeTimeout != nil && out.EntryTimeout() == 0 {
				out.SetEntryTimeout(*b.options.NegativeTimeout)
			}
			b.options.Logger.Printf("get parent  errno != 0  %v \n", errno != 0)
			return
		}

		if parent.nodeId == fuse.FUSE_ROOT_ID {
			if child == nil {
				return
			} else {

				if child.nodeId == fuse.FUSE_ROOT_ID {
					return
				}

				b.mu.Lock()
				if b.kernelNodeIds[child.nodeId] != nil {
					child = b.kernelNodeIds[child.nodeId]
				} else {
					b.kernelNodeIds[child.nodeId] = child
				}
				b.mu.Unlock()

				child, _ = b.addNewChild(root, name, child, nil, 0, out)
		...
		b.mu.Lock()
		if b.kernelNodeIds[parent.nodeId] != nil {
			parent = b.kernelNodeIds[parent.nodeId]
		} else {
			b.kernelNodeIds[parent.nodeId] = parent
		}
		if b.kernelNodeIds[child.nodeId] != nil {
			child = b.kernelNodeIds[child.nodeId]
		} else {
...
func (b *rawBridge) Open(cancel <-chan struct{}, input *fuse.OpenIn, out *fuse.OpenOut) fuse.Status {
	...
	if op, ok := n.ops.(NodeOpener); ok {
		...
		f, flags, errno := op.Open(&fuse.Context{Caller: input.Caller, Cancel: cancel, ConnectionFd: b.serverFd}, input.Flags)
		if errno != 0 {
			return errnoToStatus(errno)
		}

		if (flags & fuse.FOPEN_PASSTHROUGH) != 0 {
			if fileOps, ok := f.(FileBacker); ok {
				fileFd := fileOps.GetBackFileId()
				if fileFd > 0 {
					out.BackingId = fileFd
