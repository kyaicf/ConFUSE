...
func doInit(server *Server, req *request) {
	...
	outFlags := uint64(input.Flags) & (CAP_ASYNC_READ | CAP_BIG_WRITES | CAP_FILE_OPS |
		CAP_READDIRPLUS | CAP_NO_OPEN_SUPPORT | CAP_PARALLEL_DIROPS | CAP_MAX_PAGES | CAP_RENAME_SWAP)

	if server.opts.EnableLocks {
		outFlags |= CAP_FLOCK_LOCKS | CAP_POSIX_LOCKS
	}
	if server.opts.EnableSymlinkCaching {
		outFlags |= CAP_CACHE_SYMLINKS
	}
	if server.opts.EnableAcl {
		outFlags |= CAP_POSIX_ACL
	}
	if server.opts.EnablePassthrough {
		outFlags |= CAP_PASSTHROUGH
	...
	if server.opts.SyncRead {
		...
		outFlags &= ^uint64(CAP_ASYNC_READ)
	}
	if server.opts.DisableReadDirPlus {
		// Clear CAP_READDIRPLUS
		outFlags &= ^uint64(CAP_READDIRPLUS)
	}
	...
	outFlags |= uint64(dataCacheMode)

	if input.Minor >= 13 {
		server.setSplice()
	}

	if (input.Flags & CAP_INIT_EXT) != 0 {
		outFlags |= CAP_INIT_EXT
		server.kernelSettings.Flags2 = uint32(outFlags >> 32)
	}
	server.kernelSettings.Flags = uint32(outFlags)
