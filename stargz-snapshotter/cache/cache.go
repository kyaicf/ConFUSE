...
import (
	...
	expirLru "github.com/hashicorp/golang-lru/v2/expirable"
)
...
func NewMemoryCache(capacity int, timeout int) BlobCache {

	cache := expirLru.NewLRU[any, any](capacity, nil, time.Duration(timeout)*time.Second)
...
// LRUCacheWithLock is an LRU cache with per-key locking.
type LRUCacheWithLock struct {
	cache, _ *expirLru.LRU[any, any]
}

// NewLRUCacheWithLock creates a new LRUCacheWithLock with the given capacity.
func NewLRUCacheWithLock(capacity int, onEvict func(key any, val any), timeout int) (*LRUCacheWithLock, error) {
	c := expirLru.NewLRU[any, any](capacity, onEvict, time.Duration(timeout)*time.Second)

	return &LRUCacheWithLock{
		cache: c,
	}, nil
}

// Get retrieves the value for a given key from the cache.
func (c *LRUCacheWithLock) Get(key interface{}) (interface{}, bool) {
	return c.cache.Get(key)
}

// Set adds or updates a value in the cache for a given key.
func (c *LRUCacheWithLock) Set(key, value interface{}) {
