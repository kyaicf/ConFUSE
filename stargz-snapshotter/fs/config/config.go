...
type Config struct {
	...
	AsyncTocDownload bool `toml:"async_toc_download"`
	...
	// DirectoryCacheConfig is config for directory-based cache.
	MemoryCacheConfig `toml:"memory_cache"`
...
type MemoryCacheConfig struct {
	// Capacity is the number of entries of LRU cache to cache data on memory. Default is 256.
	Capacity int `toml:"capacity"`

	// timout is the number of seconds of LRU cache to cache data on memory. Default is 3 mins.
...
type FuseConfig struct {
	...
	// Dir store backing files
	BackingfileStorageDir string `toml:"backingfile_storage_dir"`

	// file larger than threshold will be passthroughed
	BackingFileSizeThreshold int64 `toml:"backingfile_size_threshold"`

	NonBlockPoolCapacity int `toml:"nonblock_pool_capacity"`

	StargzStoreMode bool

	FuseDebug bool `toml:"fuse_debug"`
	// Dir store toc metadata
	TocMetaStorageDir string `toml:"tocMeta_storage_dir"`
}
