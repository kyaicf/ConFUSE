...
func newCache(root string, cacheType string, cfg config.Config) (cache.BlobCache, error) {
	if cacheType == memoryCacheType {
		capacity := cfg.MemoryCacheConfig.Capacity
		if capacity == 0 {
			capacity = defaultCapacity
		}
		timeout := cfg.MemoryCacheConfig.Timeout
		if timeout == 0 {
			timeout = defaultTimeout
		}
		return cache.NewMemoryCache(capacity, timeout), nil
	}
...
func (r *Resolver) Resolve(db *reader.BackFileDb, ctx context.Context, hosts source.RegistryHosts, refspec reference.Spec, desc ocispec.Descriptor, esgzOpts ...metadata.Option) (_ Layer, retErr error) {
	defer trace.StartRegion(ctx, "fs     Resolver.Resolve").End()
	...
	additionalDecompressors := []metadata.Decompressor{new(zstdchunked.Decompressor), new(lz4chunked.Decompressor)}
	...
	meta, err := r.metadataStore(sr, r.config.AsyncTocDownload,
		append(esgzOpts, metadata.WithTelemetry(&telemetry), metadata.WithDecompressors(additionalDecompressors...))...)
	if err != nil {
		return nil, err
	}
	vr, err := reader.NewReader(meta, fsCache, desc.Digest, db)
	if err != nil {
	...
	l := newLayer(r, desc, blobR, vr, db.Cfg)
	r.layerCacheMu.Lock()
...
func (l *layer) SetTocMeta() {
	l.verifiableReader.SetTocMeta()
}
...
func (l *layer) RootNode(baseInode uint32) (fusefs.InodeEmbedder, error) {
	...
	return newNode(l.desc.Digest, l.r, l.blob, baseInode, l.resolver.overlayOpaqueType, l.bfCfg)
}
