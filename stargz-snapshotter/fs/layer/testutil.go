	...
	for _, tt := range tests {
		for srcCompressionName, srcCompression := range srcCompressions {
			...
			t.Run("testPrefetch-"+tt.name+"-"+srcCompressionName, func(t *testing.T) {
				...
				mr, err := factory(sr, false, metadata.WithDecompressors(cl))
				if err != nil {
					t.Fatalf("failed to create metadata reader: %v", err)
				}
				defer mr.Close()
				vr, err := reader.NewReader(mr, mcache, digest.FromString(""), nil)
				if err != nil {
...
func makeNodeReader(t *testing.T, contents []byte, chunkSize int, factory metadata.Store, cl tutil.Compression) (_ *file, closeFn func() error) {
	...
	r, err := factory(sr, false, metadata.WithDecompressors(cl))
	if err != nil {
		t.Fatalf("failed to create reader: %v", err)
	}
	rootNode := getRootNode(t, r, OverlayOpaqueAll, tocDgst, cache.NewMemoryCache(512, 100))
	...
	for _, tt := range tests {
		for _, srcCompression := range srcCompressions {
			...
			t.Run(tt.name, func(t *testing.T) {
				...
				r, err := factory(io.NewSectionReader(testR, 0, sgz.Size()), false, metadata.WithDecompressors(cl))
				if err != nil {
					t.Fatalf("failed to create reader: %v", err)
				}
				defer r.Close()
...
func getRootNode(t *testing.T, r metadata.Reader, opaque OverlayOpaqueType, tocDgst digest.Digest, cc cache.BlobCache) *node {
	vr, err := reader.NewReader(r, cc, digest.FromString(""), nil)
	if err != nil {
		t.Fatalf("failed to create reader: %v", err)
	}
	...
	rootNode, err := newNode(testStateLayerDigest, rr, &testBlobState{10, 5}, 100, opaque, config.FuseConfig{})
	if err != nil {
