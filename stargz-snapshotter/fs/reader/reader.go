	...
	r.ForeachChild(dirID, func(name string, id uint32, mode os.FileMode) bool {
		...
		for nr < e.Size {
			...
			eg.Go(func() error {
				...
				b := vr.r.bufPool.Get().(*bytes.Buffer)
				b.Reset()
				b.Grow(int(chunkSize))
				data := b.Bytes()[:chunkSize]

				var cacheid = genID(id, chunkOffset, chunkSize)

				if r, err := vr.r.cache.Get(cacheid); err == nil {

					n, err := r.ReadAt(data, 0)
					if (err == nil || err == io.EOF) && int64(n) > 0 {
						ip := data[:n]
						vr.saveToBackingFile(id, ip, chunkOffset)
						vr.r.bufPool.Put(b)
						r.Close()
						return nil
					}
...
func (vr *VerifiableReader) saveToBackingFile(id uint32, data []byte, offset int64) error {
	if len(data) == 0 {
		return nil
	}

	if !vr.r.BkDb.Cfg.Passthrough {
		return nil
	}
	...
	attr, err := vr.r.Metadata().GetAttr(id)
	if err != nil {
		return err
	}
	...
	if attr.Size <= BackingFileSizeThreshold {
		return nil
	}

	name, err := vr.r.Metadata().GetFileName(id)
	if err != nil {
		return err
	}

	layer := vr.r.LayerDigest()
	task := TaskArgs{data: tmp, offset: offset, layer: layer}
	task.attr = attr
	task.name = name
	task.created = time.Now().UnixNano()

	return SubmitTask(vr.r.BkDb, &task, false)
...
func (vr *VerifiableReader) SetTocMeta() error {
	if r := vr.r.r.WaitReady(); r != nil {
		return fmt.Errorf("metareader is closed")
	}

	return vr.r.SetTocMeta()
}
...
func NewReader(r metadata.Reader, cache cache.BlobCache, layerSha digest.Digest, db *BackFileDb) (*VerifiableReader, error) {
	vr := &reader{
		BkDb:  db,
		r:     r,
		cache: cache,
...
func (gr *reader) LayerDigest() string {
	return gr.layerSha.String()
}

func (gr *reader) SetTocMeta() error {
	layer := gr.layerSha.String()
	ktocPath, err := gr.BkDb.GetTocMetaPath(layer)
	if err != nil {
		return err
	}

	tocbuffer, ktocbuffer, err := gr.r.TocMetadata()
	if err != nil {
		return err
	}

	if len(ktocbuffer) == 0 {
		log.G(context.Background()).Warnf("SetTocMeta skiped %v ktocbuffer is empty\n", layer)
		return nil
	}

	if gr.BkDb.Cfg.FuseDebug {
		tocPath := path.Join(gr.BkDb.Cfg.TocMetaStorageDir, layer) + ".toc"
		err = estargz.DumpToFile(tocbuffer, tocPath)
		if err != nil {
			log.G(context.Background()).Errorf("SetTocMeta dump toc to file %v \n", err)
		} else {
			log.G(context.Background()).Debugf("dump toc to file %v \n", tocPath)
		}
	}

	err = estargz.DumpToFile(ktocbuffer, ktocPath)
	if err != nil {
		return err
	}

	return gr.BkDb.SetTocMeta(layer)
}
...
func (sf *file) ReadAt(p []byte, offset int64) (int, error) {
	t := trace.StartRegion(context.Background(), "fs reader reader file.ReadAt")
	defer t.End()
	nr := 0
	for nr < len(p) {
		...
		if r, err := sf.gr.cache.Get(id); err == nil {
			t1 := trace.StartRegion(context.Background(), "fs reader reader file.ReadAt from cache")
			readAt := time.Now().UnixNano()
			n, err := r.ReadAt(p[nr:int64(nr)+expectedSize], lowerDiscard)
			t1.End()
			if (err == nil || err == io.EOF) && int64(n) == expectedSize {

				ip := p[nr : int64(nr)+expectedSize]
				sf.writeToBackingfile(ip, lowerDiscard, readAt)
				nr += n
				r.Close()
				continue
			}
		...
		if lowerDiscard == 0 && upperDiscard == 0 {
			...
			t3 := trace.StartRegion(context.Background(), "fs reader reader file.ReadAt missed cache from underlying reader")
			readAt := time.Now().UnixNano()
			...
			sf.writeToBackingfile(ip, chunkOffset, readAt)
			nr += n
			continue
		}
		...
		b.Grow(int(chunkSize))
		ip := b.Bytes()[:chunkSize]
		t4 := trace.StartRegion(context.Background(), "fs reader reader file.ReadAt temporally cache ")
		defer t4.End()
		readAt := time.Now().UnixNano()
...
func (sf *file) writeToBackingfile(data []byte, offset int64, created int64) error {
	t := trace.StartRegion(context.Background(), "fs reader reader file.writeToBackingfile")
	defer t.End()

	// skip
	if !sf.gr.BkDb.Cfg.Passthrough {
		return nil
	}
	...
	attr, err := sf.gr.Metadata().GetAttr(sf.id)
	if err != nil {
		return nil
	}
	...
	layer := sf.gr.LayerDigest()
	task := TaskArgs{layer: layer, data: tmp, offset: offset}
	task.attr = attr

	name, err := sf.gr.Metadata().GetFileName(sf.id)
	if err != nil {
		return nil
	}
	task.name = name
	...
	return SubmitTask(sf.gr.BkDb, &task, true)

}

func (gr *reader) verifyAndCache(entryID uint32, ip []byte, chunkDigestStr string, cacheID string) error {
	t := trace.StartRegion(context.Background(), "fs reader reader reader.verifyAndCache")
	defer t.End()
