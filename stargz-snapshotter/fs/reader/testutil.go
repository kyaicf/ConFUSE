...
func makeFile(t *testing.T, contents []byte, chunkSize int, factory metadata.Store, comp tutil.Compression) (*file, func() error) {
	...
	mr, err := factory(sr, false, metadata.WithDecompressors(comp))
	if err != nil {
		t.Fatalf("failed to create reader: %v", err)
	}
	vr, err := NewReader(mr, cache.NewMemoryCache(512, 100), digest.FromString(""), nil)
	if err != nil {
		mr.Close()
			...
			for _, invalidChunkAfterVerify := range [2]bool{true, false} {
				for srcCompressionName, srcCompression := range srcCompressions {
					...
					t.Run(name, func(t *testing.T) {
						...
						mr, err := factory(sr, false, metadata.WithDecompressors(srcCompression))
						if err != nil {
		...
		t.Run(fmt.Sprintf("%v", srcCompressionName), func(t *testing.T) {
			for _, rs := range []bool{true, false} {
				for _, vs := range []bool{true, false} {
					...
					mcache := cache.NewMemoryCache(512, 100)
					mr, err := factory(io.NewSectionReader(br, 0, stargzFile.Size()), false, metadata.WithDecompressors(srcCompression))
					if err != nil {
						t.Fatalf("failed to prepare metadata reader")
					}
					defer mr.Close()
					vr, err := NewReader(mr, mcache, digest.FromString(""), nil)
					if err != nil {
						t.Fatalf("failed to make new reader: %v", err)
					}
					defer vr.Close()
	...
	for _, tt := range tests {
		for srcCompresionName, srcCompression := range srcCompressions {
			...
			t.Run(tt.name+"-"+srcCompresionName, func(t *testing.T) {
				...
				mr, err := factory(io.NewSectionReader(testR, 0, esgz.Size()), false, metadata.WithDecompressors(srcCompression))
				if err != nil {
					t.Fatalf("failed to create new reader: %v", err)
				}
				defer mr.Close()
				memcache := cache.NewMemoryCache(512, 100)
				vr, err := NewReader(mr, memcache, digest.FromString(""), nil)
				if err != nil {
					t.Fatalf("failed to make new reader: %v", err)
				}
