...
func NewFilesystem(root string, cfg config.Config, opts ...Option) (_ snapshot.FileSystem, err error) {
	...
	if cfg.FuseConfig.Passthrough {
		if cfg.FuseConfig.BackingfileStorageDir == "" {
			cfg.FuseConfig.BackingfileStorageDir = path.Join(root, "bfStorage")
		}
		if cfg.FuseConfig.TocMetaStorageDir == "" {
			cfg.FuseConfig.TocMetaStorageDir = path.Join(root, "tocMeta")
		}
	}

	db, err = reader.GetBackFileDb(cfg.FuseConfig)
	if err != nil {
		return nil, err
	}
	...
	return &filesystem{
		...
		asyncTocDownload:      cfg.AsyncTocDownload,
		debug:                 cfg.Debug,
...
func (fs *filesystem) Mount(ctx context.Context, mountpoint string, labels map[string]string) (retErr error) {
	...
	go func() {
		...
		for _, s := range src {
			l, err := fs.resolver.Resolve(fs.DB, ctx, s.Hosts, s.Name, s.Target)
			if err == nil {
				resultChan <- l
	...
	for _, desc := range neighboringLayers(preResolve.Manifest, preResolve.Target) {
		...
		go func() {
			...
			l, err := fs.resolver.Resolve(fs.DB, ctx, preResolve.Hosts, preResolve.Name, desc)
			if err != nil {
	...
	fs.DB.SetMountFd(digest.String(), server.MountFd())
	// 适配crio，如果是crio，设置mountfd
	go l.SetTocMeta()
...
func (fs *filesystem) Unmount(ctx context.Context, mountpoint string) error {
	...
	reader.RemoveLayer(l.Info().Digest.String())
...
func (fs *filesystem) prefetch(ctx context.Context, l layer.Layer, defaultPrefetchSize int64, start time.Time) {
	defer trace.StartRegion(ctx, "fs   filesystem  Resolver.Resolve").End()
