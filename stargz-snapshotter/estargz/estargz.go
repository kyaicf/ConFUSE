...
func Open(sr *io.SectionReader, opt ...OpenOption) (*Reader, error) {
	t := trace.StartRegion(context.Background(), "estargz estargz Open")
	defer t.End()
	var opts openOpts
	for _, o := range opt {
	...
	for _, d := range decompressors {
		...
		_, tocOffset, kernelTocOffset, tocSize, err := d.ParseFooter(footer[fOffset:])
		if err != nil {
		...
		r, err = parseTOC(d, sr, tocOffset, kernelTocOffset, tocSize, maybeTocBytes, opts)
		if err == nil {
			found = true
			break
		}
	...
	if err := r.initFields(true); err != nil {
		return nil, fmt.Errorf("failed to initialize fields of entries: %v", err)
	}
	return r, nil
}

func OpenWithJtoc(jtoc *JTOC) (*Reader, error) {

	r := &Reader{toc: jtoc}

	if err := r.initFields(false); err != nil {
		return nil, fmt.Errorf("failed to initialize fields of entries: %v", err)
	}
	return r, nil
}
...
func OpenFooter(sr *io.SectionReader) (tocOffset int64, footerSize int64, rErr error) {
	...
	for _, d := range []Decompressor{new(GzipDecompressor), new(LegacyGzipDecompressor)} {
		fSize := d.FooterSize()
		fOffset := positive(int64(len(footer)) - fSize)
		_, tocOffset, _, _, err := d.ParseFooter(footer[fOffset:])
		if err == nil {
...
func (r *Reader) initFields(needOffset bool) error {
...
func (r *Reader) KernelToc() (*JTOC, *KernelToc) {
	return r.toc, r.kernelToc
}
...
func (r *Reader) OpenFile(name string) (*io.SectionReader, error) {
	t := trace.StartRegion(context.Background(), "estragz estragz Reader.OpenFile")
	defer t.End()
...
func (fr *fileReader) ReadAt(p []byte, off int64) (n int, err error) {
	t := trace.StartRegion(context.Background(), "estragz estragz fileReader.ReadAt")
	defer t.End()
	if off >= fr.size {
		return 0, io.EOF
	}
...
func Unpack(sr *io.SectionReader, c Decompressor) (io.ReadCloser, error) {
	...
	blobPayloadSize, _, _, _, err := c.ParseFooter(footer)
	if err != nil {
...
func parseTOC(d Decompressor, sr *io.SectionReader, tocOff, kernelTocOffset, totalTocSize int64, tocBytes []byte, opts openOpts) (*Reader, error) {
	if tocOff < 0 {
		...
		start := time.Now()
		toc, ktoc, tocDgst, err := d.ParseTOC(nil, kernelTocOffset-tocOff, totalTocSize-(kernelTocOffset-tocOff))
		if err != nil {
			return nil, err
		}
	...
	if len(tocBytes) > 0 {
		start := time.Now()
		toc, ktoc, tocDgst, err := d.ParseTOC(bytes.NewReader(tocBytes), kernelTocOffset-tocOff, totalTocSize-(kernelTocOffset-tocOff))
		if err == nil {
			if opts.telemetry != nil && opts.telemetry.DeserializeTocLatency != nil {
				opts.telemetry.DeserializeTocLatency(start)
			}
	...
	start := time.Now()
	tocBytes = make([]byte, totalTocSize)
