...
func (gz *GzipDecompressor) ParseTOC(r io.Reader, estargzTocSize, kernelTocSize int64) (toc *estargz.JTOC, ktoc *estargz.KernelToc, tocDgst digest.Digest, err error) {
	if r != nil {
		return nil, nil, "", fmt.Errorf("TOC must be provided externally but got internal one")
	}
	rawTOC, err := gz.getTOC()
	if err != nil {
		return nil, nil, "", fmt.Errorf("failed to get TOC: %v", err)
	}
...
func parseTOCEStargz(r io.Reader) (toc *estargz.JTOC, ktoc *estargz.KernelToc, tocDgst digest.Digest, err error) {
	...
	if h.Name != estargz.TOCTarName {
		return nil, nil, "", fmt.Errorf("TOC tar entry had name %q; expected %q", h.Name, estargz.TOCTarName)
	}
	...
	if h.Name != estargz.KTOCTarName {
		return nil, nil, "", fmt.Errorf("TOC tar entry had name %q; expected %q", h.Name, estargz.KTOCTarName)
	}
