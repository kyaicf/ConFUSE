...
func Build(tarBlob *io.SectionReader, opt ...Option) (_ *Blob, rErr error) {
	...
	go func() {
		var r io.ReadCloser
		var err error
		lz4 := ctx.Value("CompressionMode")
		if lz4 != nil && lz4 == "lz4" {
			// 用cmd 封装
			// github.com/pierrec/lz4/v4 实现的reader不够健壮，构建镜像时解压存在问题。
			r, err = Lz4CmdReader(ctx, io.TeeReader(io.MultiReader(append(rs, tocAndFooter)...), pw))
		} else {
