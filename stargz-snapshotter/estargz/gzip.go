...
func (gc *GzipCompressor) WriteTOCAndFooter(w io.Writer, off int64, toc *JTOC, diffHash hash.Hash) (digest.Digest, error) {
	...
	// convert ktoc
	ktoc, err := ConvertTocToKTOC(toc)
	if err != nil {
		return "", err
	}

	if ktoc == nil {
		//maybe it is empty layer
		if err := tw.WriteHeader(&tar.Header{
			Typeflag: tar.TypeReg,
			Name:     KTOCTarName,
			Size:     int64(0),
	...
	if err := tw.WriteHeader(&tar.Header{
		Typeflag: tar.TypeReg,
		Name:     KTOCTarName,
		Size:     int64(len(ktocData)),
	}); err != nil {
		return "", err
	}
	if _, err := tw.Write(ktocData); err != nil {
		return "", err
	}
...
func (gz *GzipDecompressor) ParseTOC(r io.Reader, estargzTocSize, kernelTocSize int64) (toc *JTOC, ktoc *KernelToc, tocDgst digest.Digest, err error) {
	return parseTOCEStargz(r)
}

func (gz *GzipDecompressor) ParseFooter(p []byte) (blobPayloadSize, tocOffset, kernelTocOffset, tocSize int64, err error) {
	if len(p) != FooterSize {
		return 0, 0, 0, 0, fmt.Errorf("invalid length %d cannot be parsed", len(p))
	}
	...
	if si1 != 'S' || si2 != 'G' {
		return 0, 0, 0, 0, fmt.Errorf("invalid subfield IDs: %q, %q; want E, S", si1, si2)
	}
	if slen := binary.LittleEndian.Uint16(subfieldlen); slen != uint16(16+len("STARGZ")) {
		return 0, 0, 0, 0, fmt.Errorf("invalid length of subfield %d; want %d", slen, 16+len("STARGZ"))
	}
	if string(subfield[16:]) != "STARGZ" {
		return 0, 0, 0, 0, fmt.Errorf("STARGZ magic string must be included in the footer subfield")
	}
	tocOffset, err = strconv.ParseInt(string(subfield[:16]), 16, 64)
	if err != nil {
		return 0, 0, 0, 0, fmt.Errorf("legacy: failed to parse toc offset: %w", err)
	}
	return tocOffset, tocOffset, tocOffset, 0, nil
}
...
func (gz *LegacyGzipDecompressor) ParseTOC(r io.Reader, estargzTocSize, kernelTocSize int64) (toc *JTOC, ktoc *KernelToc, tocDgst digest.Digest, err error) {
	return parseTOCEStargz(r)
}

func (gz *LegacyGzipDecompressor) ParseFooter(p []byte) (blobPayloadSize, tocOffset, kernelTocOffset, tocSize int64, err error) {
	if len(p) != legacyFooterSize {
		return 0, 0, 0, 0, fmt.Errorf("legacy: invalid length %d cannot be parsed", len(p))
	}
	zr, err := gzip.NewReader(bytes.NewReader(p))
	if err != nil {
		return 0, 0, 0, 0, fmt.Errorf("legacy: failed to get footer gzip reader: %w", err)
	}
	defer zr.Close()
	extra := zr.Header.Extra
	if len(extra) != 16+len("STARGZ") {
		return 0, 0, 0, 0, fmt.Errorf("legacy: invalid stargz's extra field size")
	}
	if string(extra[16:]) != "STARGZ" {
		return 0, 0, 0, 0, fmt.Errorf("legacy: magic string STARGZ not found")
	}
...
func parseTOCEStargz(r io.Reader) (toc *JTOC, ktoc *KernelToc, tocDgst digest.Digest, err error) {
	zr, err := gzip.NewReader(r)
	if err != nil {
		return nil, nil, "", fmt.Errorf("malformed TOC gzip header: %v", err)
	}
	zr.Multistream(false)
	tr := tar.NewReader(zr)
	h, err := tr.Next()
	if err != nil {
		return nil, nil, "", fmt.Errorf("failed to find tar header in TOC gzip stream: %v", err)
	}
	if h.Name != TOCTarName {
		return nil, nil, "", fmt.Errorf("TOC tar entry had name %q; expected %q", h.Name, TOCTarName)
	}
	dgstr := digest.Canonical.Digester()
	toc = new(JTOC)
	if err := json.NewDecoder(io.TeeReader(tr, dgstr.Hash())).Decode(&toc); err != nil {
		return nil, nil, "", fmt.Errorf("error decoding TOC JSON: %v", err)
	}
	...
	if h.Name != KTOCTarName {
		return nil, nil, "", fmt.Errorf("TOC tar entry had name %q; expected %q", h.Name, KTOCTarName)
	}
	...
	ktoc, err = ReadKernelToc(ktocBuff, int64(len(data)))
	if err != nil {
